...
while True:
    ...
    battery_level = [random.randint(0, 100) for _ in range(10)]  # Battery Level in percentage
    gps_accuracy = [random.randint(0, 100) for _ in range(10)]  # GPS Accuracy in percentage
    connection_strength = [random.randint(-100, 0) for _ in range(10)]  # Connection Strength/Quality in dBm
    communication_status = random.choices(['Stable', 'Unstable', 'Lost'], k=10)

    # Create a dataframe for System Health and Alert
    system_health_df = pd.DataFrame(list(zip(agent_name, battery_level, gps_accuracy, connection_strength, communication_status)), columns=system_health_columns)
